...
$(OUTPUT_EN): $(TEXFILES_EN)
	$(TEX) -interaction=nonstopmode -halt-on-error $(TEXFILES_EN)
	$(TEX) -interaction=nonstopmode -halt-on-error $(TEXFILES_EN)

# Rule to build the French PDF
$(OUTPUT_FR): $(TEXFILES_FR)
	$(TEX) -interaction=nonstopmode -halt-on-error $(TEXFILES_FR)
	$(TEX) -interaction=nonstopmode -halt-on-error $(TEXFILES_FR)
...
docker-en:
	...
	$(DOCKER_RUN) sh -c "pdflatex -interaction=nonstopmode -halt-on-error $(TEXFILES_EN) && pdflatex -interaction=nonstopmode -halt-on-error $(TEXFILES_EN) && rm -f *.aux *.bbl *.blg *.log *.out *.toc"
	@echo "English resume compiled: $(OUTPUT_EN)"

docker-fr:
	@echo "Building French resume with Docker..."
	$(DOCKER_RUN) sh -c "pdflatex -interaction=nonstopmode -halt-on-error $(TEXFILES_FR) && pdflatex -interaction=nonstopmode -halt-on-error $(TEXFILES_FR) && rm -f *.aux *.bbl *.blg *.log *.out *.toc"
